...
# Replicate each list 1000 times for a larger dataset
positive_comments *= 5000
negative_comments *= 5000
neutral_comments *= 5000

# Create labels for each list: 1 for positive, 0 for negative, 2 for neutral
positive_labels = [1] * len(positive_comments)
negative_labels = [0] * len(negative_comments)
neutral_labels = [2] * len(neutral_comments)
...
chunk_size = len(comments) // size
start = rank * chunk_size
end = (rank + 1) * chunk_size if rank != size - 1 else len(comments)

local_comments = comments[start:end]
local_labels = labels[start:end]

# Start timer for global MPI execution
start_time = MPI.Wtime()

# Vectorize the comments using CountVectorizer (local computation)
vectorizer = CountVectorizer(stop_words='english')
X_local = vectorizer.fit_transform(local_comments)
y_local = np.array(local_labels)

# Train a local MultinomialNB model on each process
model = MultinomialNB()
model.fit(X_local, y_local)

# Each process predicts on its local data
local_predictions = model.predict(X_local)

# Gather predictions and labels at root process for evaluation
all_predictions = comm.gather(local_predictions, root=0)
all_labels = comm.gather(y_local, root=0)

# Root process evaluates the combined predictions
if rank == 0:
    # Combine predictions and labels from all processes
    ...
    # Make a prediction for a new comment
    test_comment = ["The video is not satisfactory"]
    test_comment_vector = vectorizer.transform(test_comment)
    prediction = model.predict(test_comment_vector)
    sentiment_map = {0: "Negative", 1: "Positive", 2: "Neutral"}
    print(f"Prediction for '{test_comment[0]}': {sentiment_map[prediction[0]]}")

# End timer for global MPI execution
end_time = MPI.Wtime()

# Print execution time (only on root process)
if rank == 0:
    execution_time_ms = (end_time - start_time) * 1000
    print(f"MPI execution time: {execution_time_ms:.2f} milliseconds")
